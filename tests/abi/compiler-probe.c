...
_Static_assert(__BYTE_ORDER__ == __ORDER_BIG_ENDIAN__, "big-endian compiler required");
#else
_Static_assert(__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__, "little-endian compiler required");
#endif
#endif
_Static_assert(sizeof(((config_t *)0)->coefficients) == 4 * sizeof(float), "array stride");
_Static_assert(sizeof(((config_t *)0)->matrix) == 4 * sizeof(int16_t), "matrix stride");
int mint_abi_probe(config_t *config, data_t *data) {
