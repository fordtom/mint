...
| `ref`      | string, unsigned integer or array | Same-block target path, `"NULL"`, or absolute target address; arrays form reflists. Mutually exclusive with other sources.                   |
...
| scalar `ref`       | `u16`, `u32`, `u64` | no                         | Same-block path, `"NULL"`, or absolute unsigned literal; fixed-point not allowed |
...
ptrs = { ref = ["table.entries", "NULL", "table.count", 0x40001000], type = "u32", size = 8 }
```
...
Resolved path address: `start_address + field_offset_octets / address_unit_octets`. An unsigned integer literal is already an absolute address in target address units; mint does not rebase or convert it. `"NULL"` is reserved and stores address 0; it is not a field path. Every address must fit the ref's `u16`, `u32` or `u64` storage type.
