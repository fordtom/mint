...
For fixed-point `qI.F` / `uqI.F` types, and the storage-width aliases `iNqF` / `uNqF`, mint always scales by `2^F` and rounds to nearest with ties to even before checking the storage range. Non-finite values are always rejected.
